...
drv_pv_base::drv_pv_base(sc_module_name& module_name) : 
  drv_pv_base_mb_compatibility(module_name),
  drv_pv_base_parameters(this),
...
void drv_pv_base::reset_registers() {
  // reset all registers
  unsigned u = 0;
...
drv_pv_base_parameters::drv_pv_base_parameters(sc_object* object) :
  m_object(object),
  ...
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, DriverFunction, "simple"),
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, AddrDist, "constant 0"),
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, SizeDist, "constant 1"),
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, WaitDist, "constant 1"),
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, NumberPackets, 10),
  SD_INITIALIZE_PARAMETER_EXTERNALLY(object, MaxPacketSize, 100)
{
  if(verbose_parameters) print_parameters();
}

void drv_pv_base_parameters::print_parameters()
{
  const char* name;
...
void drv_t_base::schedule_generic_READ
(long port_index,
 sc_dt::uint64 delay,
...
void drv_t_base::schedule_generic_WRITE
(long port_index,
 sc_dt::uint64 delay,
 long& transactionSize,
...
void drv_t_base::start_transaction_generic_READ
...
void drv_t_base::end_transaction_generic_READ
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
void drv_t_base::start_transaction_generic_WRITE
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
void drv_t_base::end_transaction_generic_WRITE
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
drv_t_base::TransactionCallbackFunctionPointer drv_t_base::getTransactionCallback(unsigned callbackIndex) {
  static TransactionCallbackFunctionPointer funcArray[] = {
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::start_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::start_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::start_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::start_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::end_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::end_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::end_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&drv_t_base::end_transaction_generic_WRITE),
    0
  };
...
config::real80 drv_t_base::get_state_power() { 
...
drv_t_base::drv_t_base(sc_module_name& module_name, long simulation) :
  ...
  m_simulation(simulation)
{
  ...
  registerParameter("DriverFunction", "const char*", sdGetParameterAsConstString("DriverFunction"));
  registerParameter("AddrDist", "const char*", sdGetParameterAsConstString("AddrDist"));
  registerParameter("SizeDist", "const char*", sdGetParameterAsConstString("SizeDist"));
  registerParameter("WaitDist", "const char*", sdGetParameterAsConstString("WaitDist"));
  registerParameter("NumberPackets", "unsigned int", sdGetParameterAsConstString("NumberPackets"));
  registerParameter("MaxPacketSize", "unsigned int", sdGetParameterAsConstString("MaxPacketSize"));
  set_sync_all(0); 
...
void drv_t_base::update_sync_all() {
  set_sync_all(0);
  
}

void drv_t_base::update_pipeline_parameters(esl::sc_sim::pipeline_policy* handle) {
...
void drv_t_base::update_sequential_parameters(esl::sc_sim::sequential_policy* handle) {
  if (m_sequential_vector.size() == 0)
    return;

}

void drv_t_base::update_delay_parameters(esl::sc_sim::delay_policy* handle) {
  if (m_delay_vector.size() == 0)
    return;

}

void drv_t_base::update_bus_parameters(esl::sc_sim::uniform_bus_policy* handle) {
  if (m_bus_vector.size() == 0)
    return;

}

void drv_t_base::update_power_parameters(esl::sc_sim::power_policy* handle) {
  if (m_power_vector.size() == 0)
    return;

}

void drv_t_base::update_state_power_parameters(esl::sc_sim::state_power_policy* handle) {
  if (m_state_power_vector.size() == 0)
    return;

}

void drv_t_base::update_global_power_parameters(esl::sc_sim::constant_global_power_policy* handle) {
  
  
  return;  
}

bool drv_t_base::triggerRegistersGotHit(unsigned portIndex, tlm::tlm_generic_payload& trans) {
  mb::utl::Segment<uint64_t> transactionRange(trans.get_address(), trans.get_address() + trans.get_data_length());

  return false;
}

bool drv_t_base::portHasRegisters(unsigned portIndex) {
  switch (portIndex) {
...
drv_pvt::drv_pvt(sc_module_name module_name)
  : esl::sc_sim::PVTBaseModel(module_name, 0),
    drv_pvt_param_defaults(this->name()),
    y("y"),
    s("s"),
...

{
  ...
  m_drv_t = drv_t_base::create_t("T", 1);
  m_drv_pv = new drv_pv("PV");
  bind();
}  
...
drv_pvt::drv_pvt(sc_module_name module_name, sc_core::sc_time clock_init, sc_core::sc_time generic_clock_init, double nominal_voltage_init, bool mb_debug_init, bool call_to_default_if_init, bool verbose_parameters_init, bool dmi_enabled_init, const char* warning_level_init, unsigned int s_pipeline_length_init, const char* DriverFunction_init, const char* AddrDist_init, const char* SizeDist_init, const char* WaitDist_init, unsigned int NumberPackets_init, unsigned int MaxPacketSize_init)
  : esl::sc_sim::PVTBaseModel(module_name, 0),
  drv_pvt_param_defaults(this->name(), clock_init, generic_clock_init, nominal_voltage_init, mb_debug_init, call_to_default_if_init, verbose_parameters_init, dmi_enabled_init, warning_level_init, s_pipeline_length_init, DriverFunction_init, AddrDist_init, SizeDist_init, WaitDist_init, NumberPackets_init, MaxPacketSize_init),
    y("y"),
    s("s"),
    y_entry("unvisible_y_entry"),
    s_entry("unvisible_s_entry"),
...
drv_pvt::drv_pvt(sc_module_name module_name,  std::vector<std::pair<char*, unsigned int> > &change_parameters)
  : esl::sc_sim::PVTBaseModel(module_name, 0),
  drv_pvt_param_defaults(this->name(), change_parameters),
    y("y"),
    s("s"),
    y_entry("unvisible_y_entry"),
    s_entry("unvisible_s_entry"),
...
drv_pvt::~drv_pvt() {
  ...
  delete m_drv_t; delete m_drv_pv;
}
...
void drv_pvt::bind()
{
  ...
  m_drv_t->check_mb_events();
}



void drv_pvt::b_transport_outside_y(tlm::tlm_generic_payload& p, sc_core::sc_time& t)
{
  if (!y.get_interface()) {
...
unsigned int drv_pvt::transport_dbg_outside_y(tlm::tlm_generic_payload& p)
{
  if (!y.get_interface()) {
     return 0;
  }
