...
class WaveSampleGenerator_pv_base;
class WaveSampleGenerator_pv_base_mb_compatibility : public mb::mb_module {
  friend class WaveSampleGenerator_pv_base;
 public:
  typedef unsigned int mb_address_type;
 protected:
  WaveSampleGenerator_pv_base_mb_compatibility(sc_core::sc_module_name& module_name)
...
class WaveSampleGenerator_pv_base_parameters {
 public :
  WaveSampleGenerator_pv_base_parameters(sc_object* object);
...
class WaveSampleGenerator_pv_base : public WaveSampleGenerator_pv_base_mb_compatibility,
                           public WaveSampleGenerator_pv_base_parameters
{
 private:
  typedef WaveSampleGenerator_pv_base_mb_compatibility mb_compatibility_class;
  typedef WaveSampleGenerator_pv_base_mb_compatibility BASE_TYPE;
 public:
  typedef esl::tlm_types::Address mb_address_type;
 public:
  typedef WaveSampleGenerator_pv_base self;
  SC_HAS_PROCESS(WaveSampleGenerator_pv_base);
  WaveSampleGenerator_pv_base(sc_core::sc_module_name& module_name);
 ...
 public:
  ...
  tlm::tlm_target_socket <16, tlm::tlm_base_protocol_types>  Slave;  
...
class WaveSampleGenerator_t_base : public papoulis::SystemCBaseModel {
public:
  static WaveSampleGenerator_t_base* create_t(const char* _name, long simulation);
public:
  WaveSampleGenerator_t_base(sc_core::sc_module_name& module_name, long simulation);
...
protected:
  friend class WaveSampleGenerator_model;
...
public:
  enum variable_enum {Waveform_idx };
protected:
  mb::mb_t_variable<int> Waveform; 
...
class WaveSampleGenerator_pv;



using namespace std;

class WaveSampleGenerator_pvt_param_defaults {
  
public:
  WaveSampleGenerator_pvt_param_defaults(const char* hier_name) {
    ...
    Max = ::mb::sysc::sdInitParameter<unsigned int>("Max", USHRT_MAX, hier_name);
    Min = ::mb::sysc::sdInitParameter<unsigned int>("Min", 0, hier_name);
    Period = ::mb::sysc::sdInitParameter<sc_core::sc_time>("Period", sc_core::sc_time(1.0, sc_core::SC_MS), hier_name);
    Type = ::mb::sysc::sdInitParameter<const char*>("Type", "sine", hier_name);
    ::mb::sysc::sdPropagateParameters();
  }

  WaveSampleGenerator_pvt_param_defaults(const char* hier_name, sc_core::sc_time clock_init, sc_core::sc_time generic_clock_init, double nominal_voltage_init, bool mb_debug_init, bool call_to_default_if_init, bool verbose_parameters_init, bool dmi_enabled_init, const char* warning_level_init, unsigned int Slave_pipeline_length_init, unsigned int Max_init, unsigned int Min_init, sc_core::sc_time Period_init, const char* Type_init) {
    ...
    Max = ::mb::sysc::sdInitParameter<unsigned int>("Max", Max_init, hier_name);
    Min = ::mb::sysc::sdInitParameter<unsigned int>("Min", Min_init, hier_name);
    Period = ::mb::sysc::sdInitParameter<sc_core::sc_time>("Period", Period_init, hier_name);
    Type = ::mb::sysc::sdInitParameter<const char*>("Type", Type_init, hier_name);

    ::mb::sysc::sdPropagateParameters();
    }

  WaveSampleGenerator_pvt_param_defaults(const char* hier_name, std::vector<std::pair<char*, unsigned int> > &change_parameters){
...
class WaveSampleGenerator_pvt : public esl::sc_sim::PVTBaseModel, 
                          public mb::utl::cb_owner,
                          private WaveSampleGenerator_pvt_param_defaults {

 public:
  typedef WaveSampleGenerator_pvt self_type;

 public: 
  // Constructor
  WaveSampleGenerator_pvt(sc_core::sc_module_name module_name); 
  ...
  WaveSampleGenerator_pvt(sc_core::sc_module_name module_name, std::vector<std::pair<char*, unsigned int> > &change_parameters);
  ~WaveSampleGenerator_pvt(); 

 
 public:
    //access the T instance
  inline WaveSampleGenerator_t_base* getT() const {
    return m_WaveSampleGenerator_t;
  }
  
  //access the PV instance
  inline WaveSampleGenerator_pv* getPV() const {
    return m_WaveSampleGenerator_pv;
  }
 ...
 private:
  ...
  //T instance
  WaveSampleGenerator_t_base* m_WaveSampleGenerator_t;
  
  //PV instance
  WaveSampleGenerator_pv* m_WaveSampleGenerator_pv;

 public:
  // External ports

  typedef esl::sc_sim::pvt_target_socket<16, tlm::tlm_base_protocol_types, 1, sc_core::SC_ONE_OR_MORE_BOUND> Slave_type;
  Slave_type Slave;

 private:
  // PV connection ports

  typedef mb::tlm20::initiator_socket_cb<16> Slave_entry_type;
 ...
 private:
  ...
  struct Slave_nb_connector : public tlm::tlm_bw_nonblocking_transport_if<>, public ::mb::tlm20::socket_tracer_holder {
    WaveSampleGenerator_pvt& m_owner;
    
    Slave_nb_connector(WaveSampleGenerator_pvt& owner) : m_owner(owner) {
    }
